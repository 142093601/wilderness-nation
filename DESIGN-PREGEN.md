...
> ⚠️ **单位更正（2026-09-23 实测）**：`/chunky radius N` 的 **N 是"方块"，不是"区块"**。
> 证据：`/chunky radius 200` → 日志报 **729 区块**（27×27，= 432 方块见方 ✓）；
> `/chunky radius 24` → **25 区块**（5×5）。所以想要"半径 64 区块"必须写
> **`/chunky radius 1024`**。下面表格的"覆盖范围"一列就是**该填进命令的数字**。

voxy 视距 `sectionRenderDistance=16` ⇒ `16 × 32 = 512 区块 = 8192 方块`。

| 覆盖范围（= `/chunky radius` 的值） | 区块数 | 按 12/秒（**本机实测**） | 按 20/秒（纯生成，估） |
|---|---:|---:|---:|
| 1024 方块（= 64 区块半径） | 16,641 | **约 23 分钟** | 0.2 h |
| **2048 方块（= 128 区块半径）** | 66,049 | **约 1.5 小时** | 0.9 h |
| 4096 方块 | 263,169 | 约 6 小时 | 3.7 h |
| 8192 方块（= 满视距） | 1,050,625 | **约 24 小时** | 14.6 h |

> **结论：想"整片预生成好"是 24 小时起步，不现实。**
> 现实做法是**只预生成核心区**，外围仍靠边玩边长。

**速率是实测的**：`/chunky radius 200` 那次 343 区块用了约 28 秒 ⇒ **约 12 区块/秒**
（日志自己也报 `Rate: 12.1 cps`）。这个数字比"估计"可靠，用它换算即可。

**为什么要预生成（2026-09-23 实测复现）**：把玩家瞬移进**从未生成过**的区域
（`scdev` 的 x≈3000/z≈3000），服务端立刻报
`Can't keep up! Running 14849ms or 296 ticks behind` —— **一次 14.8 秒的巨停**，
与需求方自己那次 **17.9 秒**同级。⇒ 探索新地形的巨停就是"边走边生成"的代价，
预生成买的就是把它挪到你有空的时候。
（反面也确认过：**进世界后头一分钟的卡顿不是生成**，见 `DESIGN-PERF.md` §11.4 —— 那个预生成治不了。）

### 5.2 做法（cheats 已开，实测三个世界 `allowCommands=1`）

在游戏里执行（Chunky 命令语法取自 jar 的 `lang/en.json`）：

```
/chunky center <x> <z>    # 不写就沿用上次的中心
/chunky radius 2048       # ← 单位是方块！2048 方块 = 128 区块半径
/chunky start
/chunky progress          # 查进度（日志里也会周期性打 Rate/ETA）
