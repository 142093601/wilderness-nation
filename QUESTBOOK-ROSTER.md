...
### 4.1.2 跨章重复判据的**处置与结论**（2026-09-22）

规矩只在 **mod 级**被守住了，**物品级从来没有工具把关** —— 于是实测出
**541 对共享判据**，其中 **21 对"标题 + 判据完全相同"的真重复**。
根因是我派了 15 个 agent 分头写，**每个 agent 都守规矩，合起来却重了**，
而我在派活之前没有先建这个检查。检查已补：`tools/questbook_dup_check.py`。

**处置原则（不删任务、不改 name）**：拥有该 mod 的那一章保留 `count=1`（"第一次拿到"），
另一章改成明确的**批量**（"攒够一批"）。这样语义各自成立、不再同时亮，
而且 **name / title / 依赖全不动 → id 不变 → 玩家进度不倒退**。
只有确实是纯重复的才删（本次删 1 条：`90-blueprints/engineer_goggles`，
并把 `build_by_blueprint` 对它的依赖一起去掉）。

**结果**：真重复 **21 → 0**；全书 1777 → 1776 条。

**这个检查器自己踩过两个假阳性，都写在 `tools/questbook_dup_check.py` 的注释里**：

1. **图标不算判据**。第一版把 `icon` 收进来，于是"图标撞了但判据不同"的两条被判成重复
   （`154-cat-structures` 与 `45-expedition` 的「鉴定神器」实际用的是两个不同进度）。
   差一点去改坏两条本来就对的任务。
2. **数量要算进判据**。`count=1` 与 `count=8` 不是同一个要求，做完 1 个只点亮前者。
   不计数量的话，"用数量区分两章"这种正当处置在报告里看起来像根本没修。

**教训**：**判据的定义就是检查器的全部**。定义错了，检查器会一本正经地报假警，
而假警会引导人去"修"本来正确的内容 —— 那比不检查更危险。
