...
| **任务书数据（全 40 章）** | ✅ **完成**（2026-09-22）：**40 章 / 1777 任务**。详见 §3.6 |
| **任务书架构修订（22 → 40 章）** | ✅ **完成**（2026-09-22）：实测本机三个成熟包后推翻了初版"22 章"的判断，见 §3.6 |
...
> ✅ **2026-09-22：40 章已全部铺完**（1777 任务）。原来的 1~2 步已不再适用，保留作历史记录。

1. ~~请用户开游戏看手感~~ → **等用户验收**（任务书已装进实例，按 N 就能看）
2. ~~按模板铺其余 17 章~~ → **已完成**（40 章齐）

### 3.6 这一轮做完的事（2026-09-22）：任务书 437 → 1777 条

**起因是用户的两次反馈**：
1. 「任务书界面不错，但内容量太少，一章才 10 个任务左右」—— 我当时以为要加厚**文案**
2. 「重点是去学习本机上其他整合包任务线怎么写的，**这个任务量太少了**……
   40 章左右完全不多了」—— 用户自己纠正了上一轮的判断

**做法（先取证再动手）**：写了 `tools/reference_questbook_style.py`，
**只统计结构、不抄文字**（ATM10 的任务书是 ARR，我们仓库公开）：

| 包 | 章 | 任务 | 每章中位 | 最大章 |
|---|---|---|---|---|
| All the Mods 10 | 63 | 4528 | **56** | 253 |
| 天空蜂巢 | 35 | 2901 | **66** | 336 |
| 龙之冒险 | 14 | 1566 | **58** | 492 |
| **本包（修订前）** | 23 | 437 | **14** | 68 |
| **本包（现在）** | **40** | **1777** | **~45** | 95 |
...
真正的问题是**每章是空壳**。于是定 40 章 / 每章 40~56 条，
并补上**形状语义**（五边形=簇头 · 圆角方块=主轴 · 八角=里程碑 · 六边形=终局）。

**同时修好的工具与坑**（每条都记进 `INCIDENTS.md`）：
- `layout()` 改**迭代式不动点**：旧版裸递归在依赖成环时 `RecursionError` 把生成器整个打挂
- 新增**跨章软链接** `[[link]]` → `quest_links`（两遍编译解析目标 id）
- `questbook_check` 第⑦项补**图标**校验；新增第⑩项**真机拒绝过的 id**
- 新增 `item_index.py`（查真实 id）· `runtime_deny.py` + `runtime_deny_fix.py`
  （真机坏 id 的发现 → 修复闭环）· `lang_drift_check.py`（语言文件与 TOML 漂移）·
  `questbook_name_guard.py`（name 消失 = id 变 = 进度回退）· `toml_quote_fix.py` ·
  `reference_questbook_style.py`
- **最大的一个坑**：`registry.json` 证明的是"某个 jar 提到过这个 id"，
  **不等于"游戏注册了它"** —— 离线报 missing 0，服务端报 **37 条** `Unknown registry key`。
  **唯一可信的证据是真机日志**，所以有了第 ⑩ 项与 deny list。

**并行作业的教训**：一次派了 15 个 agent，**9 个中途失败**（但失败者已经写了文件）。
处置是"先盘点再补"而不是重做；并发上限压到 3~5。
...
2. **看一眼任务书**（22 章已就位）并给腔调意见 —— 打开方式：**按 N** 或 `/ftbquests open_book`
   （实测拿到的命令名；书有没有发到手都不影响这条命令可用）。
...
| ~~中~~ | ~~**AppCDS（类数据共享）**~~ | ❌ **已测，决定性负优化**：`-XX:+AutoCreateSharedArchive` 的"记录类"环节让世界就绪 **+242 s**（基线中位 ~144）、客户端可玩 **+325 s**（基线 195~200），且归档 ~390 MB（194 万对象）dump 超过 30 秒优雅退出窗口、**没写出来**。见 `DESIGN-PERF.md` §十六。**JVM 参数层到此收口**（Aikar ❌ / CDS ❌ / 堆 ⛔ 内存不够）|
...
- ~~❌ **左侧 40~60 章** —— 否决（"一堵墙"），定 22 章~~
  → ✅ **2026-09-22 撤销**：这条否决是**基于错误的量级判断**做的。
  实测本机整合包：**ATM10 = 63 章 / 3746 任务**、天空蜂巢 28 章 / 2283 任务、
  龙之冒险 5 章 / 996 任务。ATM10 是市面上最流行的整合包之一，63 章并不"堵墙"。
  **新结论：40 章左右完全不多，目标定在 40~50 章量级。**
  （用户原话：*"atm 原来有这么多章吗，那是我判断失误了，那我觉得 40 章左右完全不多了，
  所以现在我觉得还是得继续加"*）
- ❌ **抄 ATM-10 的任务内容** —— 其 quests 目录是 **All Rights Reserved**，我们仓库公开，只能作结构参照
- ❌ **重复 mod 自带教学**（Create Ponder、JEI/EMI）—— 仍有效；我们的 desc 只讲"为什么需要/在流程里的位置"
- ❌ **任务专属内容**、**硬门闸**（除时代钥匙）
