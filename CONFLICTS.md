...
### T19 · 「进世界」判据又假阳性了一次，这次骗的是 `game_agent` 的**功能脚本**（🔴 已修）

**C13 修的是 `autotest` 的验收协议，这一条是同一类错误的第二个入口**：功能脚本（`game_agent --script`）
在发命令之前也等 `autotest.wait_for_in_world`，而那个函数认的是

```
LoggerChunkProgressListener | Preparing spawn area | Time elapsed:
```

前两个都是**开始**生成区域时写的（`LoggerChunkProgressListener` 第一行就是"2%"），不是完成。

**证据（2026-09-19 20:11 启动的那一局，装 statecraft 后的第一次功能测试）**：

| 时刻 | 事件 |
|---|---|
| 20:14:07 | `LoggerChunkProgressListener/]: 准备生成区域中：2%` ← **判据在这里就成立了** |
| 20:14:19 / :22 / :34 | 脚本的 3 条命令（`/statecraft info` ×2、`/statecraft roundtrip`）全部发出 |
| 20:14:22 | `LoggerChunkProgressListener/]: Time elapsed: 16442 ms` ← 真正的载入完成 |
| 20:14:56 | 本 mod 的 `ServerStartedEvent`（`Statecraft 建库：seed=-6066409030490083615`）← 服务端**这时**才真的起来 |
| 20:14:59 | `IntegratedServer/]: Changing view distance to 12, from 10` ← 客户端这时才可交互 |
| 20:15:00 | `Can't keep up! Running 3898160ms or 77963 ticks behind` 然后进程被关 —— 脚本早就"跑完"了 |

**后果有两层**：
1. 直接后果：3 条命令**一条都没进到服务端**（日志里没有 `[Server thread]` 的命令派发），
   脚本报 `0/3`，看起来像"mod 功能坏了"；
2. 隐蔽后果：脚本"跑完"后随即退出，**游戏在世界加载完成 4 秒后被杀**，
   于是 `saves/<world>/data/statecraft.dat` 根本没落盘 —— 而这一条很容易被误读成
   "存档没写成功"，实际是**根本没到存档那一步**。

**已修（两处，缺一不可）**：

| 位置 | 改动 |
|---|---|
| `autotest.LEVEL_LOADED_MARKER` | 只认**完成**标记 `Time elapsed:`。刻意不用 `准备生成区域中：x%`：那行跟随客户端语言（zh_cn 下不是英文），且是开始标记 |
| `game_agent.wait_for_input_ready`（新增） | 等日志标记**不够**：实测从 `Time elapsed` 到能交互还差 ~37 秒。改成**闭环探针**——发一条普通聊天（`sc-probe-N`）并等它自己出现在日志里，出现才认为按键真的被收到了。用聊天而不是命令，是为了不依赖任何 mod 注册命令、也不受客户端语言影响 |
| `game_agent` 进世界等待上限 | 300 → **420 秒**（这个包从启动到服务端真正起来要 ~230 秒，原来只剩 70 秒余量） |

**顺带必须记一笔（未处理）**：既然"发命令太早"能发生，那么**过去"通过"的功能脚本结果都要重新审视** ——
比如 `opac_v0.txt` 里 `expect: Open Parties and Claims` 这种字符串，在 mod **加载横幅**里也会出现，
所以哪怕命令没发进去，断言也可能"碰巧"匹配成功。判定准则应该是**只接受命令自己的回显**
（像本 mod 的 `STATECRAFT_INFO` 这类前缀标记，或 `opac_v0.txt` 里那些`Claims: \d+ / \d+` 的结构化行）。

#### T19 追加（同一天，第二次跑）：修完判据后仍然是 0/3，真凶是**死亡屏**

第二次跑（20:19 启动，判据已改成 `Time elapsed:` + 闭环探针）：
- `20:21:59 Time elapsed: 13235 ms` → 判据正确触发；
- `20:22:31 Statecraft 建库` → 服务端起来；
- `20:23:10 saves/b5v3/data/statecraft.dat`（1038 字节）落盘 → **mod 侧存档链路是通的**；
- 但探针 `sc-probe-N` 一条都没进日志，连手工发的 `win_ctl.ps1 -Action cmd -Value "sc-manual-probe-1"`
  也没进（`RAN cmd len=17`，说明**消息确实投递出去了**）。

**截一张图就真相大白**（`data/shots/probe1.png`）：玩家躺在死亡屏上 ——
「你死了 / niuniu_jiang 被卫道士杀死了 / 重生 / 标题屏幕」。
**死亡屏吞掉所有按键**，所以 `T`（开聊天）+ 打字 + 回车全部无效。

**教训（这条比 bug 本身值钱）**：功能脚本报 0/N 时，**先截一张图**，再怀疑功能。
这次的 0/3 有**三层**原因叠在一起，任何一层单独看都会得出错误结论：
① 判据假阳性（T19 主条，已修）→ 命令发在服务端起来之前；
② 测试世界 `b5v3` 是危险世界，玩家开局就被卫道士杀了 → 死亡屏；
③ 脚本"跑完"立刻退出 → 游戏在世界加载完成 4 秒后被杀 → 看起来像"存档没落盘"。

**已做的处置**：M2 的落盘/读档证据**不依赖按键通道**（`建库`/`读档成功` 是服务端启动时打的日志，
玩家死不死都会打），所以改走"两次启动 + 日志比对"，见 `PLAN-statecraft-02-persist.md`。
**未做**：死亡屏自动恢复（需要在 `win_ctl.ps1` 里加鼠标点击，或把测试世界设成和平+创造）。
**下次要跑功能脚本之前，先把测试世界设成和平、玩家设成创造** —— 否则这个坑会再吃一次。

**新增取证工具**：`tools/nbt_dump.py` —— 自己读 gzip+NBT（不拉 nbtlib 依赖），
把 `saves/<world>/data/statecraft.dat` 打成 JSON，用来核对"存档到底写对了没有"。
它给出的证据见 `PLAN-statecraft-02-persist.md` 的验收表。

---

### T20 · `autotest.py --world` **从来就没生效过**，而表头一直在打印你传的那个世界（🔴 已修）

同一类"自动化悄悄做了别的事"的第三个入口，也是危害最大的一个。

**代码事实（`tools/autotest.py`）**：`cfg` 是从 `DEFAULTS` 拷出来的，但拷贝用的键列表里**没有 `world`**：

```python
cfg = dict(DEFAULTS)
for key in ("version_dir", "version_id", "game_root", "java", "username", "uuid", "xmx",
            "width", "height"):          # ← 这里没有 "world"
    cfg[key] = getattr(args, key)
...
print(f"world   : {args.world}")          # ← 表头打印的是参数，不是真正用来启动的值
```

而启动命令用的是 `cfg["world"]`（`build_command` 里 `"quickPlaySingleplayer": cfg["world"]`），
所以 **`--world X` 永远是空操作**：命令里始终是 `DEFAULTS["world"]` = **`新的世界`**，
表头却照实打印你传的 `--world X`。2026-09-19 20:27 复现：表头 `world : b5v3`，
而打印出来的 `game args` 里赫然是 `--quickPlaySingleplayer 新的世界`。

**确定受影响的**：所有经 `autotest.py --world` 的路径，包括 `paired_check.py` 的客户端侧测试
（它 `copytree` 到 `saves/<world>` 后只靠 `--world` 告诉 autotest 用哪个世界）。
**后果**：批次的**客户端侧"进世界"证据，实际是在玩家真实世界「新的世界」上取得的**，
不是准备好的 `bN-world`。两点要分开看：
- 仍然成立的部分：批次 mod 集合的**类路径装载**与"这个包能不能进世界"是同一份实例，
  而且真实世界比新世界更重，所以"能进世界、无崩溃"这一条**不因此失效**；
- **失效的部分**：一切"载入的是那个准备好的批次世界"的说法，以及任何依赖**世界内容**的断言。
- **副作用（记一笔）**：这些客户端测试是用**逐批的部分 mod 集合**去开玩家真实存档的，
  理论上存在"世界内容被部分 mod 集合改写"的风险（本轮没有出现 `丢失方块` 一类报告，但风险是真实的）。

**不确定的部分（诚实标注）**：`data/paired.log` 只有 2.2 KB、不含启动参数，
`copytree` 又会**保留文件 mtime**，所以**无法从现存产物反推**每次运行到底开了哪个世界。
因此"具体哪几次批次的结论受影响"不能凭现有证据定量，只能说：**凡是把结论挂在"准备好的批次世界"上的，
都要按修正后的协议重跑一遍**。批次重跑本来就已经在待办里（见 §八）。

**已修（两处，缺一不可）**：
1. `world` 加进拷贝键列表 —— 让 `--world` 真的生效；
2. 表头改成打印 `cfg["world"]` —— **表头必须打印真正用来启动的那个值**，
   否则它只会替 bug 打掩护（这次就是它让 bug 活了这么久）。

**方法论教训（与 C13/T19 同源，第三次了）**：这个项目里"看起来在验证 A、实际验证了 B"已经出现三次
（C13 的崩溃后仍 PASS、T19 的命令发太早、T20 的世界不是那个世界）。
