...
（`PLAN-questbook.md` §4.4）。

**已在用**（2026-09-22 起）。TOML 写法：

```toml
[[link]]
to_chapter = "create"      # 目标**章**的 key（不是 id）
to_quest = "first_crank"   # 目标章内那条任务的 name（不是 id）
x = -12.0                  # 可选：在本章画布上的位置（缺省排在左侧、按序号往下错开）
y = -3.5
```

生成物（实测自天空蜂巢的 chapter 文件，字段名一致）：

```
quest_links: [
	{
		id: "24B82F1FA748DEEF"          # 链接自己的 16 位 id（生成器发）
		linked_quest: "29C3777502434B67" # **目标任务 id**（不是章 id）
		x: -12.0d
		y: -3.5d
	}
]
...
- `linked_quest` 指向**任务**，所以生成器必须**两遍**编译：先全部章拿到 id，
  再统一解析链接（`Compiler.apply_links()`）。目标不存在**直接报错** ——
  写错的指路牌在游戏里完全不报错，只会把玩家引到空白处。
- 软链接**不构成解锁条件**：目标任务没做也能看，做了也不解锁这里。
- 指向本章请用 `deps`（那才是真前置）；`to_chapter` 写成自己会报错。
