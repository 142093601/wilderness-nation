...
## 2026-09-23 我把 ModernFix 的"JVM 跑了多久"当成了"加载花了多久"

### 现象

同一天、同一台机器、同一个包、同一个存档：受控跑（`perf_baseline.py`）三次都得到
**世界就绪 +125~132 秒**，而手工那次（`autotest.py`）是 **+194.8 秒**。
第一反应是"那次被污染了"，但日志检验（时间戳单调、只有一次会话）说没污染。

去对时间戳，才发现**我引用的那三个数字量的根本不是加载阶段**：

| 行 | 时刻（novc 那次会话，`2026-09-23-6.log.gz`）|
|---|---|
...
1. `Game took` 的**终点是"它被打印的那一刻"**（世界就绪之后 26 秒），零点反推是进程启动
   ⇒ 它量的是"JVM 运行了多久"，里面混着世界就绪之后的空转。
2. `Total` = `Game took` + `main menu to in-game`（159.575 + 61.685 = 221.260，小数完全对上）
   ⇒ 它不是独立的第三个数字。

**后果**：`DESIGN-PERF.md` 里 base/aikar/novc 的"启动到主菜单 158.6/154.6/159.6 秒"
整张表作废（已按日志时间戳改写为 +130.5/+125.3/+131.9 秒）。
更麻烦的是我拿它们做过两个 A/B —— 幸好那两个结论另有依据
（Aikar 的否决依据是 GC 日志的**最坏暂停 319→3101 ms**，与口径无关，结论仍然成立）。

### 修法

1. 口径换成**日志时间戳**：JVM 第一行 → `Reloading ResourceManager`
   → `Starting integrated minecraft server` → `Time elapsed:`（世界就绪）。逐行可查。
2. `perf_baseline.py` 调 `perf_report.py` 时**必须同时传 `--log`**：
   以前只传 `--gc-log`，于是**保存下来的报告里"加载各阶段"永远是空的**
   （它把 GC 日志当主日志解析了）。
3. 阶段数字只从**报告表格列**里取，不再去正文抠句子
   （第一版抠 `共 **143** 秒`，而实际排版是整句加粗 ⇒ 永远匹配不到、JSON 静默留空）。

### 硬规则（新增）

25. **引用的数字必须能自己复算。** 拿不到计时零点的日志行（`Game took` 这类）
    不许当作阶段耗时：每个数字都要能指到"哪一行、相对哪一行、差多少"。
26. **不同启动方式（工具 / 参数 / 分辨率）的会话不进同一张对照表。**
    同一配置重复跑的极差要先量出来（本轮实测三次受控跑 ±5%）；
    改动效果小于这个极差就不能算"有效"。
27. **日志归档（`.gz`）一律用能解压的读法。** `perf_gaps.py` 曾经直接 `read_text()`
    读 `.gz`（= 把压缩字节按 utf-8 解码），4176 行的会话只解析出 2200 行乱码，
    并且**报出"0 个空档"** —— 它差点让我得出"那次慢与 I/O 无关"的结论。
    **检查器口径错了，绿灯比红灯更贵。**

### 附带翻车：凭先验写的"最大帧数开销"被实测否掉

`DESIGN-PERF.md §6.2` 原来写着 `graphicsMode 2→1`（Fabulous→Fancy）是
"单项最大的帧数开销"。实测正午场景：**GPU 只占 34%，帧数撞在 120 上限**
（`119 / 120 fps 垂直同步`）⇒ 这个场景是 **CPU 瓶颈**，Fabulous 的额外开销不显形。
**没测之前不要把自己写的建议当成事实**（详见 `DESIGN-PERF.md` §8）。