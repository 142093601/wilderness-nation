...
# 等"游戏真的进世界"多久（秒）。
#
# ⚠️ 2026-09-20 实测：**这个包要 15 分钟以上才进世界**（232 个 mod；Exposure 等几个 mod
# 启动时还会去拉远程名单，而这台机器连不上、每次都要等到连接超时，光这一项就吃掉好几分钟）。
# 第一次给 420 秒、第二次给 900 秒，两次都停在 "Minecraft: NeoForge Loading..." /
# 资源包重载那一屏上，报出来是"未能进入世界"——看起来像客户端坏了，其实是等得不够久。
WORLD_READY_SECONDS = 1800.0
